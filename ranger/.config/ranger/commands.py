...
class fd(Command):
    ...
    def execute(self):
        ...
        if self.arg(1):
            ...
            self.fm.notify(":fd needs a query.", bad=True)
            return
...
class fd_next(Command):
    """
    :fd_next
    Selects the next match from the last :fd.
    """

    def execute(self):
        if len(fd.SEARCH_RESULTS) > 1:
            fd.SEARCH_RESULTS.rotate(-1)  # rotate left
            self.fm.select_file(fd.SEARCH_RESULTS[0])
        elif len(fd.SEARCH_RESULTS) == 1:
            self.fm.select_file(fd.SEARCH_RESULTS[0])
...
class fd_prev(Command):
    ...
    def execute(self):
        if len(fd.SEARCH_RESULTS) > 1:
            fd.SEARCH_RESULTS.rotate(1)  # rotate right
            self.fm.select_file(fd.SEARCH_RESULTS[0])
        elif len(fd.SEARCH_RESULTS) == 1:
            self.fm.select_file(fd.SEARCH_RESULTS[0])
