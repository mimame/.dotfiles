...
#                          ^-- calendar week
status.register("clock", format=" %a %-d %b %T KW%V")

status.register("uptime", format=" {hours}:{mins}", interval=1)
...
status.register("load", format=" {tasks} {avg1} {avg5} {avg15}")


# status.register("cpu_usage_graph",
#    start_color=base05,)

status.register("cpu_freq", format=" {avgg}", interval=1)
...
status.register(
    "temp", format=" {Package_id_0}°C", lm_sensors_enabled=True, interval=1
)
...
# 42/128G [86G]
status.register(
    "disk", path="/", format=" {avail}G", on_leftclick="baobab", interval=1
)

# pomodoro timer
status.register(
    "pomodoro", sound="/usr/share/sounds/freedesktop/stereo/bell.oga", interval=1
)
